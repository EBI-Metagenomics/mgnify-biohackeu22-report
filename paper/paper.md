...
[MGnify](https://www.ebi.ac.uk/metagenomics) [@citesAsAuthority:10.1093/nar/gkac1080] is [EMBL-EBI](https://www.ebi.ac.uk)’s metagenomics resource, which is part of the [ELIXIR Metagenomics Emerging Community](https://elixir-europe.org/communities/marine-metagenomics). 
...
This ready to use environment and analysis notebooks examples bridge the gap between the simple but necessarily limited options available when browsing the MGnify website, and the complexity but possibilities of installing a local environment to work with data stored in MGnify. 
...
![Schematic of the Notebook Server technology stack. Other configurations (e.g. different servers or Docker base layers) are also possible. \label{schematic}](./figures/notebooks-server-schematic.png)
...
The notebooks are standard Jupyter notebooks and following installation of the necesary language kernels and packages, they can be used on any computer.
...
Prior to [BioHackathon Europe 2022](https://biohackathon-europe.org/), MGnify Notebooks were deployed to one publicaly available server: a [ShinyProxy](https://www.shinyproxy.io/) server hosted by [EMBL's Bio-IT](https://bio-it.embl.de/) project.
...
This also makes the server suitable for "deep-linking": the lack of a login process means notebooks can be easily launched into a particular state using URL query parameters (see [@citesAsAuthority:10.1093/nar/gkac1080] figure 4).
...
During BioHackathon Europe 2022, we therefore developed a [Jupyter Lab user interface (UI) extension](https://jupyterlab.readthedocs.io/en/3.5.x/user/extensions.html) that places MGnify-specific help and resources into Jupyter Lab.
...
- users discovering the notebooks on Galaxy to learn more about MGnify,
- users needing quick access to MGnify's [interactive API browser](https://www.ebi.ac.uk/metagenomics/api/v1/) when fetching data in notebooks,
...
Jupyter Lab widgets are hybrid web/kernel components that can communicate between a user interface built using web-technologies and the running Jupyter kernel (e.g. Python).
...
This enables richer notebook workflows than would otherwise be possible; the default IGV behaviour is to only show a popup with the focussed annotation's details (figure \ref{igv}).
...
The code and examples used in this notebook illustrate the range of data and features in MGnify's genomes API, and allow users to re-use and build upon sample code for their own purposes. 
...
Through "code as documentation" and ready-to-use compute environments, users can accomplish short programmatic access tasks (like building a TSV file of data requiring multiple API requests) or follow more in-depth analysis workflows than are possible on the [MGnify website](https://www.ebi.ac.uk/metagenomics) (like cross-study comparisons).
